...
### Simple English Wikipedia article intros — ${wiki} passages
- **Source:** Simple English Wikipedia, via the Hugging Face datasets-server
  rows API (dataset \`${WIKI_DATASET}\`, config \`${WIKI_CONFIG}\`). Random
...
## Embeddings
- **Model:** ${MODEL_ID} (quantized ONNX, \`onnx/model_quantized.onnx\`; the
  underlying sentence-transformers/all-MiniLM-L6-v2 is Apache-2.0), run via
...
- **Storage:** \`public/data/embeddings.bin\` is float32, little-endian,
  row-major \`[count][dim]\`; \`public/data/meta.json\` records shape and dtype.
